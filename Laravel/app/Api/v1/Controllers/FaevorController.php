...
class FaevorController extends Controller implements PinInterface {
    ...
    public function update($faevor_id)
    {
        ...
        { 
            ...
            {
                $this->filesRefUpdate($faevor->file_ids, $this->request->file_ids);
                $faevor->file_ids = null;
            }
            else if($this->fileIdsExists($this->request->file_ids))
            {
                $this->filesRefUpdate($faevor->file_ids, $this->request->file_ids);
                $faevor->file_ids = $this->request->file_ids;
            }
        ...
        if($this->request->has('tag_ids'))
        {
            if($this->tagIdsExists($this->request->tag_ids))
            {
                $this->tagsRefUpdate($faevor->tag_ids, $this->request->tag_ids);
                $faevor->tag_ids = $this->request->tag_ids;
            }
            else
            {
        ...
        if($this->request->has('bouns'))
        {
            ...
            {
                $faevor->bouns = $this->request->bouns;
            }
        }
    ...
    public function delete($faevor_id)
    {
        ...
        $this->filesDeref($faevor->file_ids);
        $this->tagsDeref($faevor->tag_ids);
        $faevor->delete();
    ...
    private function updateValidation(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'bonus' => array('filled', 
                'required_without_all:file_ids,tag_ids,budget,name,description,due_time,expire_time,geo_longitude,geo_latitude', 
                'string', 'max:50'),
